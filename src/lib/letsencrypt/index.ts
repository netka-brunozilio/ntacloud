...
export async function letsEncrypt(domain: string, id?: string, isNTACloud = false): Promise<void> {
	try {
		...
		if (dualCerts) {
			...
			try {
				await asyncExecShell(
					`DOCKER_HOST=${host} docker run --rm -v "ntacloud-letsencrypt:/etc/letsencrypt" -v "ntacloud-ssl-certs:/app/ssl" alpine:latest sh -c "ls -1 /app/ssl/${wwwDomain}.pem"`
				);
				found = true;
			} catch (error) {
				//
			...
			await asyncExecShell(
				`DOCKER_HOST=${host} docker run --rm --name certbot-${randomCuid} -p 9080:${randomPort} -v "ntacloud-letsencrypt:/etc/letsencrypt" certbot/certbot --logs-dir /etc/letsencrypt/logs certonly --standalone --preferred-challenges http --http-01-address 0.0.0.0 --http-01-port ${randomPort} -d ${nakedDomain} -d ${wwwDomain} --expand --agree-tos --non-interactive --register-unsafely-without-email ${
					dev ? '--test-cert' : ''
				}`
			);
			await asyncExecShell(
				`DOCKER_HOST=${host} docker run --rm -v "ntacloud-letsencrypt:/etc/letsencrypt" -v "ntacloud-ssl-certs:/app/ssl" alpine:latest sh -c "test -d /etc/letsencrypt/live/${nakedDomain}/ && cat /etc/letsencrypt/live/${nakedDomain}/fullchain.pem /etc/letsencrypt/live/${nakedDomain}/privkey.pem > /app/ssl/${nakedDomain}.pem || cat /etc/letsencrypt/live/${wwwDomain}/fullchain.pem /etc/letsencrypt/live/${wwwDomain}/privkey.pem > /app/ssl/${wwwDomain}.pem"`
			);
			...
			try {
				await asyncExecShell(
					`DOCKER_HOST=${host} docker run --rm -v "ntacloud-letsencrypt:/etc/letsencrypt" -v "ntacloud-ssl-certs:/app/ssl" alpine:latest sh -c "ls -1 /app/ssl/${domain}.pem"`
				);
				found = true;
			} catch (error) {
				//
			...
			if (found) return;
			await asyncExecShell(
				`DOCKER_HOST=${host} docker run --rm --name certbot-${randomCuid} -p 9080:${randomPort} -v "ntacloud-letsencrypt:/etc/letsencrypt" certbot/certbot --logs-dir /etc/letsencrypt/logs certonly --standalone --preferred-challenges http --http-01-address 0.0.0.0 --http-01-port ${randomPort} -d ${domain} --expand --agree-tos --non-interactive --register-unsafely-without-email ${
					dev ? '--test-cert' : ''
				}`
			);
			await asyncExecShell(
				`DOCKER_HOST=${host} docker run --rm -v "ntacloud-letsencrypt:/etc/letsencrypt" -v "ntacloud-ssl-certs:/app/ssl" alpine:latest sh -c "cat /etc/letsencrypt/live/${domain}/fullchain.pem /etc/letsencrypt/live/${domain}/privkey.pem > /app/ssl/${domain}.pem"`
			);
			await reloadHaproxy(host);
		}
	} catch (error) {
...
export async function generateSSLCerts(): Promise<void> {
	...
	for (const application of applications) {
		try {
			if (application.fqdn && application.destinationDockerId) {
				...
				if (isRunning) {
					if (isHttps) ssls.push({ domain, id, isNTACloud: false });
				}
				if (previews) {
					...
					if (containers.length > 0) {
						for (const container of containers) {
							...
							if (isHttps) ssls.push({ domain: previewDomain, id, isNTACloud: false });
						}
					}
				}
			}
	...
	if (fqdn) {
		const domain = getDomain(fqdn);
		const isHttps = fqdn.startsWith('https://');
		if (isHttps) ssls.push({ domain, id: 'ntacloud', isNTACloud: true });
	}
	if (ssls.length > 0) {
					...
					if (ipv4 || ipv6) {
						...
						if (domains4.length > 0 || domains6.length > 0) {
							if (
								...
								return await letsEncrypt(ssl.domain, ssl.id, ssl.isNTACloud);
							}
						}
					}
