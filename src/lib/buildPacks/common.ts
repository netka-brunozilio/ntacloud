...
export function makeLabelForStandaloneApplication({
	...
	return [
		'ntacloud.managed=true',
		`ntacloud.version=${version}`,
		`ntacloud.type=standalone-application`,
		`ntacloud.configuration=${base64Encode(
			JSON.stringify({
				applicationId,
				fqdn,
				name,
...
export async function makeLabelForStandaloneDatabase({ id, image, volume }) {
	...
	return [
		'ntacloud.managed=true',
		`ntacloud.version=${version}`,
		`ntacloud.type=standalone-database`,
		`ntacloud.configuration=${base64Encode(
			JSON.stringify({
				version,
				image,
...
export function makeLabelForServices(type) {
	return [
		'ntacloud.managed=true',
		`ntacloud.version=${version}`,
		`ntacloud.type=service`,
		`ntacloud.service.type=${type}`
	];
}
