<script lang="ts">
	...
	// }
	onMount(async () => {
		if (state === false && destination.isNTACloudProxyUsed === true) {
			destination.isNTACloudProxyUsed = !destination.isNTACloudProxyUsed;
			try {
				await post(`/destinations/${id}/settings.json`, {
					isNTACloudProxyUsed: destination.isNTACloudProxyUsed,
		...
		} else if (state === true && destination.isNTACloudProxyUsed === false) {
			destination.isNTACloudProxyUsed = !destination.isNTACloudProxyUsed;
			try {
				await post(`/destinations/${id}/settings.json`, {
					isNTACloudProxyUsed: destination.isNTACloudProxyUsed,
	...
	async function changeProxySetting() {
		if (!cannotDisable) {
			const isProxyActivated = destination.isNTACloudProxyUsed;
			if (isProxyActivated) {
				const sure = confirm(
					`Are you sure you want to ${
						destination.isNTACloudProxyUsed ? 'disable' : 'enable'
					} NTACloud proxy? It will remove the proxy for all configured networks and all deployments on '${
						destination.engine
	...
	async function stopProxy() {
		try {
			...
			return toast.push('NTACloud Proxy stopped!');
		} catch ({ error }) {
			return errorNotification(error);
		}
	}
	async function startProxy() {
		try {
			...
			return toast.push('NTACloud Proxy started!');
		} catch ({ error }) {
			return errorNotification(error);
		}
	}
	async function forceRestartProxy() {
		...
		if (sure) {
			try {
				restarting = true;
				toast.push('NTACloud Proxy restarting...');
...
<form on:submit|preventDefault={handleSubmit} class="grid grid-flow-row gap-2 py-4">
	<div class="flex space-x-1 pb-5">
		...
		{#if $session.isAdmin}
			...
			>
		{/if}
		<!-- <button type="button" class="bg-netka hover:bg-netka-100" on:click={scanApps}
	...
	<div class="grid grid-cols-2 items-center">
		<Setting
			...
			bind:setting={destination.isNTACloudProxyUsed}
			on:click={changeProxySetting}
			title="Use NTACloud Proxy?"
			description={`This will install a proxy on the destination to allow you to access your applications and services without any manual configuration. Databases will have their own proxy. <br><br>${
				cannotDisable
					? '<span class="font-bold text-white">You cannot disable this proxy as FQDN is configured for NTACloud.</span>'
					: ''
			}`}
		/>
	</div>
...
<!-- <div class="flex justify-center">
	{#if payload.isNTACloudProxyUsed}
		{#if state}
