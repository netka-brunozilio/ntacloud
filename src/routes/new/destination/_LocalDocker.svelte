...
<div class="flex justify-center px-6 pb-8">
	<form on:submit|preventDefault={handleSubmit} class="grid grid-flow-row gap-2 py-4">
		...
		<div class="grid grid-cols-2 items-center px-10">
			...
			<input required name="network" placeholder="default: ntacloud" bind:value={payload.network} />
		</div>
		{#if $session.teamId === '0'}
			<div class="grid grid-cols-2 items-center">
				<Setting
					bind:setting={payload.isNTACloudProxyUsed}
					on:click={() => (payload.isNTACloudProxyUsed = !payload.isNTACloudProxyUsed)}
