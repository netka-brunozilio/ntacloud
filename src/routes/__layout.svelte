...
{#if $session.userId}
	<nav class="nav-main">
		<div class="flex h-screen w-full flex-col items-center transition-all duration-100">
			{#if !$session.whiteLabeled}
				<div class="my-4 h-10 w-10"><img src="/favicon.png" alt="Netka logo" /></div>
			{/if}
			...
			<div class="flex flex-col space-y-4 py-2">
				...
				>
					<a
						...
						href={`https://github.com/netka/ntacloud/releases/tag/v${$session.version}`}
	...
	{#if $session.whiteLabeled}
		<span class="fixed  bottom-0 left-[50px] z-50 m-2 px-4 text-xs text-stone-700"
			>Powered by <a href="https://ntacloud.io" target="_blank">NTACloud</a></span
		>
	{/if}
