...
export const get: RequestHandler = async (event) => {
	...
	try {
		...
		const containers = listContainers.filter((container) => {
			return (
				container.Labels['ntacloud.configuration'] &&
				container.Labels['ntacloud.type'] === 'standalone-application'
			);
		});
		const jsonContainers = containers
			.map((container) =>
				JSON.parse(Buffer.from(container.Labels['ntacloud.configuration'], 'base64').toString())
			)
			.filter((container) => {
				return (
