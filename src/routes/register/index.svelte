	...
	{:else}
		<div class="flex justify-center px-4">
			<form on:submit|preventDefault={handleSubmit} class="flex flex-col py-4 space-y-2">
				<div class="text-6xl font-bold border-gradient w-48 mx-auto border-b-4">NTACloud</div>
				...
				<div class="flex space-x-2 h-8 items-center justify-center pt-8">
					<button
						type="submit"
						class="hover:bg-netka-100 text-white"
						disabled={loading}
						class:bg-transparent={loading}
						class:text-stone-600={loading}
						class:bg-netka={!loading}>{loading ? 'Registering...' : 'Register'}</button
					>
				</div>
			</form>
		</div>
		{#if userCount === 0}
			<div class="pt-5">
				You are registering the first user. It will be the administrator of your NTACloud instance.
				<br />
				It will take a while, because NTACloud will configure itself, the proxy and other docker related
				stuff.
			</div>
		{/if}
	{/if}
